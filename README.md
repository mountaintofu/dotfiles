...
- ![quicksw.sh](	/../main/config/quickwg.sh)* [ depends on _WireGuard_ (available on AUR) and _Proton VPN_ (just create an account and check for WG's config files there, remember to extend the files expiration dates), don't forget to put its link in _/.local/bin/_ ]
